...
int arm_op_exec_arm_teq_imm(struct TargetCore *core,  arm_teq_imm_input_type *in, arm_teq_imm_output_type *out)
{
	int ret = 0;
	uint32 *status = cpu_get_status(core);
	out->next_address = core->pc + INST_ARM_SIZE;
	out->passed = ConditionPassed(in->cond, *status);
	if (out->passed != FALSE) {
		out->result = in->Rn.regData ^ in->imm32;
		CPU_STATUS_BIT_UPDATE(status, CPU_STATUS_BITPOS_C, out->status_flag.carry);
		CPU_STATUS_BIT_UPDATE(status, CPU_STATUS_BITPOS_Z, (out->result == 0));
		CPU_STATUS_BIT_UPDATE(status, CPU_STATUS_BITPOS_N, ((out->result & (1U << 31U)) != 0));
	}
...
int arm_op_exec_arm_sbc_imm(struct TargetCore *core,  arm_sbc_imm_input_type *in, arm_sbc_imm_output_type *out)
{
	int ret = 0;
	uint32 result;
	uint32 *status = cpu_get_status(core);
	...
	out->passed = ConditionPassed(in->cond, *status);
	if (out->passed != FALSE) {
		result = AddWithCarry(32, in->Rn.regData, ~(in->imm32), out->status_flag.carry, &out->status_flag);
		if (in->Rd.regId != CpuRegId_PC) {
			cpu_set_reg(core, in->Rd.regId, result);
			if (in->S != 0) {
				cpu_update_status_flag(status, result, &out->status_flag);
