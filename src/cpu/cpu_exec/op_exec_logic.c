...
int arm_op_exec_arm_asr_imm_a1(struct TargetCore *core)
{
	...
	in.S = op->S;
	OP_SET_REG(core, &in, op, Rd);
	DecodeImmShift(0b10, op->imm5, &in.shift_t, &in.shift_n);
	OP_SET_REG(core, &in, op, Rm);
...
int arm_op_exec_arm_asr_reg_a1(struct TargetCore *core)
{
	arm_OpCodeFormatType_arm_asr_reg_a1 *op = &core->decoded_code->code.arm_asr_reg_a1;

	arm_asr_reg_input_type in;
	arm_asr_reg_output_type out;
	out.status = *cpu_get_status(core);
	...
	int ret = arm_op_exec_arm_asr_reg(core, &in, &out);
	DBG_ARM_ASR_REG(core, &in, &out);
