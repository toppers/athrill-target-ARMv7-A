...
int arm_op_exec_arm_rrx(struct TargetCore *core,  arm_rrx_input_type *in, arm_rrx_output_type *out)
{
	int ret = 0;
	uint32 result;
	uint32 *status = cpu_get_status(core);
	...
	if (out->passed != FALSE) {
		...
		result = Shift_C(32, in->Rm.regData, SRType_RRX, 1, out->status_flag.carry, &out->status_flag.carry);
		if (in->Rd.regId != CpuRegId_PC) {
			cpu_set_reg(core, in->Rd.regId, result);
			if (in->S != 0) {
				//APSR.N = result<31>;
...
int arm_op_exec_arm_asr_imm(struct TargetCore *core,  arm_asr_imm_input_type *in, arm_asr_imm_output_type *out)
{
	...
	out->passed = ConditionPassed(in->cond, *status);
	if (out->passed != FALSE) {
		result = Shift_C(32, in->Rm.regData, SRType_ASR, in->shift_n, out->status_flag.carry, &out->status_flag.carry);
		if (in->Rd.regId != CpuRegId_PC) {
			cpu_set_reg(core, in->Rd.regId, result);
			if (in->S != 0) {
				//APSR.N = result<31>;
...
int arm_op_exec_arm_asr_reg(struct TargetCore *core,  arm_asr_reg_input_type *in, arm_asr_reg_output_type *out)
{
	int ret = 0;
	uint32 result;
	uint32 *status = cpu_get_status(core);
	...
	if (out->passed != FALSE) {
		...
		result = Shift_C(32, in->Rn.regData, SRType_ASR, shift_n, out->status_flag.carry, &out->status_flag.carry);
		out->Rd.regData = result;
...
int arm_op_exec_arm_lsr_reg(struct TargetCore *core,  arm_lsr_reg_input_type *in, arm_lsr_reg_output_type *out)
{
	...
	if (out->passed != FALSE) {
		uint32 shift_n = in->Rm.regData & 0xFF;
		result = Shift_C(32, in->Rn.regData, SRType_LSR, shift_n, out->status_flag.carry, &out->status_flag.carry);
		out->Rd.regData = result;
		cpu_set_reg(core, in->Rd.regId, out->Rd.regData);
		if (in->S != 0) {
			//APSR.N = result<31>;
